...
## Telegram-дайджест (опціонально)

Після класифікації скрипт може надіслати компактний підсумок у Telegram-чат.

1. Створи бота через [@BotFather](https://t.me/BotFather), отримай токен.
2. Дізнайся свій `chat_id` (наприклад через [@userinfobot](https://t.me/userinfobot)).
3. Додай у `.env`:
   ```
...
Якщо змінні відсутні — скрипт працює як раніше, дайджест просто не надсилається.

Для перевірки надсилання без повного прогону через Gemini (берегти денну квоту):
```bash
